...
std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> temp_cloud_list;
...
void cloudFilter(pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud_filtered, bool downsample = true)
{
	...
	{
		if   (cloud->points[i].x > -6  && cloud->points[i].x < 6
		 &&  cloud->points[i].y > -6  && cloud->points[i].y < 6   )
...
//全局计数
	...
	if ( temp_cloud->points.size() == 0 )  //第一帧点云
	{
		temp_cloud = input_cloud;
		fused_cloud = input_cloud;
		key_cloud = input_cloud;
	}
	...
	std::cout << "Euler ANGEL: \n" << Rotation2Euler(Ts.topLeftCorner<3, 3>()).transpose() 
	<< "\n" << Rotation2Euler(Ta.topLeftCorner<3, 3>()).transpose() << std::endl;

	if (fabs(Rotation2Euler(Ts.topLeftCorner<3, 3>())[1]) > KEYTH  || fabs(Rotation2Euler(Ta.topLeftCorner<3, 3>())[1]) > KEYTH)
	{
		Ta.setIdentity(4,4);
		ROS_INFO_STREAM(" The " << global_count << " frame\n");
		frame_count +=1;

		pclomp_ndtRegistration(input_cloud, key_cloud, Tb);  //点云配准， 外参矩阵
		key_cloud = input_cloud;
	...
	pcl::toROSMsg(*fused_cloud, msg_clouds);
	msg_clouds.header.frame_id = "velodyne";
...
//全局计数
	...
	// temp_cloud_list.resize(WINDOWS);
	...
	key_cloud = pcl::PointCloud<pcl::PointXYZI>::Ptr (new pcl::PointCloud<pcl::PointXYZI>);
	
	// for (int i=0; i< WINDOWS; i++)
	// {
	// 	temp_cloud_list[i] = pcl::PointCloud<pcl::PointXYZI>::Ptr (new pcl::PointCloud<pcl::PointXYZI>);
	// 	// cloud_matrix_list[i] = Eigen::Matrix4f::Identity();
	// 	// temp_image_list[i] = cv::Mat::zeros(720,1280,CV_8UC3);
	// 	// homo_list[i] = cv::Mat1f::eye(3,3);
	// 	// image_matrix_list_ess[i] = Eigen::Matrix4f::Identity();
	// 	// image_matrix_list_pnp[i] = Eigen::Matrix4f::Identity();
	// 	// rotation_error_list[i] = Eigen::Array3f::Zero();
	// 	// gt_rotation_error_list[i] = Eigen::Array3f::Zero();
	// 	T_list[i] = Eigen::Matrix4f::Identity();
	// }
